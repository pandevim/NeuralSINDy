...
4. Solve sparse regression: Ẋ ≈ Θ · ξ
5. Report which MLPs were selected and their coefficients
6. Validate by simulating the discovered system and comparing to ground truth
"""
...
from neural_sindy import build_default_library, discover_equations, NeuralSINDyLibrary
...
def main():
    ...
    library = build_default_library("checkpoints")
    ...
    # ── Save results ──
    np.savez(
        data_dir / "discovery_results.npz",
        xi=xi,
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig(plots_dir / "discovery_comparison.png", dpi=150, bbox_inches="tight")
        plt.close()
        print(f"  ✓ Comparison plot saved to {plots_dir / 'discovery_comparison.png'}")
    ...
    plt.tight_layout()
    plt.savefig(plots_dir / "coefficients.png", dpi=150, bbox_inches="tight")
    plt.close()
    print(f"  ✓ Coefficient bar chart saved to {plots_dir / 'coefficients.png'}")

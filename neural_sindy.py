"""
Phase 2: Neural SINDy — using grokked MLPs as basis functions.

Standard SINDy: Ẋ = Θ(X) · ξ
    where Θ = [1, x, x², sin(x), cos(x), ...]  (hand-crafted library)
    and ξ is a sparse coefficient vector

Neural SINDy: Ẋ = Θ_neural(X) · ξ
    where Θ = [MLP_identity(x), MLP_cos(x), MLP_sin(x), ...]  (grokked MLP library)
    and ξ is a sparse coefficient vector found via STLSQ or LASSO

The key insight: because each MLP is a differentiable function learned to
approximate a clean math operation, the sparse regression selects which
*operations* best explain the dynamics.
"""
...
def build_default_library(checkpoint_dir="checkpoints"):
    """

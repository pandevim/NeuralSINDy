"""
Experiment 4 (deterministic-val variant).

Identical to exp4 in every modeling choice — same router, same conditional slot
entropy, same alpha=1.5 complexity prior, same training schedule. The only
differences are:
...
3. exp_id = "exp4_det"
   Separate scorecard row and separate HuggingFace upload prefix.

Train behavior is byte-identical to exp4 — the noise toggle only affects val
...
def run(load_grokked_mlp, hf_repo_id, device="cpu",
    ...
    """
    Run Experiment 4 with deterministic val evaluation.
    ...
    print("=" * 60)
    print(f"  TRAINING (Exp 4 DET: Top-{k} Router + Conditional Slot Entropy, alpha={alpha})")
    print("=" * 60)
    ...
    scorecard = compute_scorecard(router, history, X_val, dXdt_val, truth, exp_id="exp4_det", noise=False)

    print_summary(router, history, summary, truth)
    plot_run(router, history, summary, exp_id="Exp 4 (det val)")

    if upload:
        upload_run(router, history, summary, exp_id="exp4_det", hf_repo_id=hf_repo_id)

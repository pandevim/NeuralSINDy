...
If standard SINDy relies on rigid basis functions, and methods like Symbolic-KAN route over exact mathematical primitives, our proposed architecture offers a fundamental departure: utilizing "grokked" Multilayer Perceptrons (MLPs) \cite{b2} as the candidate functions. This neural hybridization offers two distinct theoretical advantages over exact mathematical libraries. First, neural approximations naturally smooth idiosyncratic noise rather than propagating it deterministically. Second, the slight variance inherent in neural approximations provides a unique mathematical texture that relaxes the strict collinearity of redundant terms (e.g., an addition operator versus its distinct linear components), thereby aiding the differentiable router in resolving otherwise rank-deficient systems.
...
\begin{thebibliography}{00}
  ...
  \bibitem{b4}
  B.~M.~de~Silva, K.~Champion, M.~Quade, J.~C.~Loiseau, J.~N.~Kutz, and S.~L.~Brunton, ``PySINDy: A Python Package for the Sparse Identification of Nonlinear Dynamical Systems from Data,'' \textit{Journal of Open Source Software}, vol.~5, no.~49, p.~2104, 2020.

  \bibitem{b5}
  N.~Nanda, T.~Lieberum, K.~Singhal, M.~Balcells, J.~Dickstein, C.~Tsvetkov, P.~Atanasova, M.~Soula, S.~Mandyl, G.~Irving, V.~Nanda, A.~Scherlis, L.~Chan, Y.~Bengio, R.~Sharma, J.~Steinhardt, S.~Bowman, and E.~Hubinger, ``Progress Measures for Grokking via Mechanistic Interpretability,'' in \textit{International Conference on Learning Representations (ICLR)}, 2023.
